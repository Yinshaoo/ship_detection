...
class NSADetrTransformerEncoder(BaseModule):
    """Encoder of DETR.
    ...
    def _init_layers(self) -> None:
        ...
        self.layers = ModuleList([
            NSADetrTransformerEncoderLayer(**self.layer_cfg)
            for _ in range(self.num_layers)
        ])
        self.embed_dims = self.layers[0].embed_dims

    def forward(self, query: Tensor, query_pos: Tensor,
                key_padding_mask: Tensor,h=None,w=None, **kwargs) -> Tensor:
        ...
        for layer in self.layers:
            query = layer(query, query_pos, key_padding_mask,h,w, **kwargs)
        return query
...
class NSADetrTransformerEncoderLayer(BaseModule):
    ...
    def _init_layers(self) -> None:
        ...
        self.self_attn = NSA(**self.self_attn_cfg)
    ...
    def forward(self, query: Tensor, query_pos: Tensor,
                key_padding_mask: Tensor,h,w, **kwargs) -> Tensor:
